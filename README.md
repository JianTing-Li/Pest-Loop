...
- **Full ARIA combobox on address search, not a styled `<div>` pretending to be one.** Arrow keys, Enter, Escape, `aria-activedescendant` — all real, all tested. The autocomplete index is derived client-side from data already in memory rather than fetched separately: the full building list is downloaded before the search box ever renders, so a second file would've been pure duplicate bytes for fields that dataset already has.
...
2. **The React app** reads only those static files — address search with keyboard-accessible autosuggest, CSV batch matching, ranking/filtering, a per-building violation timeline, and a copyable renewal-brief generator, all client-side.
